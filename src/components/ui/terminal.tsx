...
/** Preload keyboard sprite so typing sounds are ready before animation starts. */
export function preloadTerminalAudio(): Promise<void> {
  if (sharedAudioBuffer && sharedAudioCtx) return Promise.resolve();
  ...
  audioLoadPromise = (async () => {
    ...
    if (!res.ok) {
      throw new Error(`Failed to load terminal sounds (${res.status})`);
    }
    sharedAudioBuffer = await ctx.decodeAudioData(await res.arrayBuffer());
    sharedAudioCtx = ctx;
...
export async function unlockTerminalAudio(): Promise<void> {
  try {
    await preloadTerminalAudio();
    if (sharedAudioCtx?.state === "suspended") {
      await sharedAudioCtx.resume();
    }
  } catch {
    // Sound file missing or autoplay blocked
  }
}

function useAudio(enabled: boolean) {
  useEffect(() => {
    if (enabled) void preloadTerminalAudio();
  }, [enabled]);

  const playSound = (sound: [number, number] | undefined) => {
    if (!enabled || !sound || !sharedAudioBuffer || !sharedAudioCtx) return;
    if (sharedAudioCtx.state === "suspended") {
...
export function Terminal({
  ...
  const [audioReady, setAudioReady] = useState(!enableSound);
  ...
  useEffect(() => {
    if (!inView || phase !== "idle" || !audioReady) return;
    const t = setTimeout(() => setPhase("typing"), initialDelay);
    return () => clearTimeout(t);
  }, [inView, phase, initialDelay, audioReady]);
